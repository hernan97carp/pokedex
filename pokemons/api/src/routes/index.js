...
const getPokeInfo = async (id) => {
  try {
    ...
    const allDescriptions = speciesresult["flavor_text_entries"].filter(
      (el) => el.language.name === "en"
    );
    const speciespok = speciesresult.genera.filter(
      (el) => el.language.name === "en"
    );
    ...
    const pokemonInfo = {
      ...
      abilities: results.abilities
        ? results.abilities.map((a) => a.ability.name)
        : null,
      growth: speciesresult["growth_rate"]
        ? speciesresult["growth_rate"].name
        : null,
      habitat: speciesresult.habitat ? speciesresult.habitat.name : null,
      description: allDescriptions[random(allDescriptions.length)][
        "flavor_text"
...
router.get("/pokemons/:idPokemon", async (req, res) => {
  ...
  let pokemonInfo;
  if (
    (idPokemon >= 1 && idPokemon <= 898) ||
    (idPokemon >= 10001 && idPokemon <= 10220)
  ) {
